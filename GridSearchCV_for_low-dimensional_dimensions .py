...
def get_eval_dict(this_output_score, epoch_type, n_hidden):
    ...
    this_dict = {'lr': lr, 'wd': wd, 'b_size': b_size, 'num_epoches': epoch_type, 'n_hidden': n_hidden, 'mean_dev_auc': mean_dev_auc,
...
class My_Net(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.dr_p_embedding = Dr_P_Embedding(n_hidden)
        self.other_embedding = Other_Embedding(n_hidden)
        self.GO_sim_embedding = GO_sim_embedding(n_hidden)
        self.GO2P = GO_to_P(n_hidden)
        self.Pathway2P = Pathway_to_P(n_hidden)
        self.All_Net = All_Graph_Net(n_hidden)
        self.connected_layer1 = nn.Sequential(nn.Linear(in_features=n_hidden * 7, out_features=256), nn.BatchNorm1d(num_features=256), nn.ReLU())
...
for n_hidden in n_hiddens:
    ...
    for i in range(5):
        ...
        best_test = []
        losses = nn.BCELoss()
        model = My_Net(n_hidden=n_hidden).to(device)
    ...
    dict_300 = get_eval_dict(output_score_epoch_300, epoch_type=300, n_hidden=n_hidden)
    record_300 = pd.DataFrame.from_dict(dict_300, orient='index').T

    if all_output_results.empty:
        all_output_results = record_300
    else:
        all_output_results = pd.concat([all_output_results, record_300])
    all_output_results.to_csv('all_records_n_hidden.csv', index=False)
