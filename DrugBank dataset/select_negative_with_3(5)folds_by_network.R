# select_negative samples with 1:3 and 1:5
# fold = 3
fold = 5
...
n_1 = matrix(nr=length(dr_id[,1])*2*fold,nc=2)
k = 1
for(i in 1:length(dr_id[,1]))
{
  ...
  if(n_negative_sample>2*fold)
  {
    random = sample(1:n_negative_sample,2*fold)
...
n_2 = matrix(nr=length(p_id[,1])*2*fold,nc=2)
k = 1
...
num_remain = n_P*fold-length(n_this_1[,1])
...
# write.csv(n_all,file = "D:/Users/czx/PycharmProjects/HNGDTI/Drugbank dataset/negative samples/neg_DTI-net_24060.csv",row.names =FALSE,quote = F)
write.csv(n_all,file = "D:/Users/czx/PycharmProjects/HNGDTI/Drugbank dataset/negative samples/neg_DTI-net_40100.csv",row.names =FALSE,quote = F)
