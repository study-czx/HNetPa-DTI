...
def Get_train_P(data, dr_id_map, p_id_map):
    P_list = []
    for i in range(len(data)):
        P_list.append([dr_id_map[data[i][0]], p_id_map[data[i][1]]])
    return P_list
    ...
    def __init__(self):
        ...
        self.Graph_Net = HGCN(
            {'d-t-dr': SAGE(in_feats=(n_hidden, n_hidden), out_feats=n_hidden,  feat_drop=drop_feat, aggregator_type=agg, activation=F.relu),
             ...
             'dr-p': SAGE(in_feats=(n_hidden, n_hidden), out_feats=n_hidden, feat_drop=drop_feat, aggregator_type=agg,activation=F.relu),
             'p-dr': SAGE(in_feats=(n_hidden, n_hidden), out_feats=n_hidden, feat_drop=drop_feat, aggregator_type=agg,activation=F.relu)},
            aggregate='sum')
...
class My_Net(nn.Module):
    ...
    def forward(self, x_dr, x_p, finger_feats, seq_feats,  disease_feat, MF_feat, BP_feat, CC_feat, MF_sim_Graph, BP_sim_Graph, CC_sim_Graph, GO2P_G, my_G):
    ...
    for type in types:
        ...
        for i in range(5):
            ...
            test_N = np.loadtxt(data_type + seed_type + "/" + type + "/test_N.csv", dtype=str, delimiter=",",
                                skiprows=1)
            train_Dr_P = Get_train_P(train_P, dr_id_map, p_id_map)
            my_G = funcs.Add_Dr_P_Net_Graph(train_Dr_P, Dr_D_m_data, Dr_D_t_data, P_D_m_data, D_D_data, P_P_data,
                                            num_nodes_dict)
            my_G = my_G.to(device)
