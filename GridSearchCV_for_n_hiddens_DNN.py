...
# first GridsearchCV for DNN1
DNN1s = [64, 128, 256, 512]
DNN2s = [128]
DNN3s = [64]

# second, select best of DNN1s, then GridsearchCV for DNN2
# DNN1s = [256]
# DNN2s = [64, 128, 256, 512]
# DNN3s = [64]

# second, select best of DNN1s and DNN2s, then GridsearchCV for DNN3
# DNN1s = [256]
# DNN2s = [128]
# DNN3s = [64, 128, 256, 512]
...
def get_eval_dict(this_output_score, DNN1, DNN2, DNN3):
    ...
    this_dict = {'DNN1': DNN1, 'DNN2': DNN2, 'DNN3': DNN3, 'mean_dev_auc': mean_dev_auc,
...
class My_Net(nn.Module):
    def __init__(self, n_DNN1, n_DNN2, n_DNN3):
        ...
        self.connected_layer1 = nn.Sequential(nn.Linear(in_features=n_hidden * 7, out_features=n_DNN1), nn.BatchNorm1d(num_features=n_DNN1), nn.ReLU())
        self.connected_layer2 = nn.Sequential(nn.Linear(in_features=n_DNN1, out_features=n_DNN2), nn.BatchNorm1d(num_features=n_DNN2), nn.ReLU())
        self.connected_layer3 = nn.Sequential(nn.Linear(in_features=n_DNN2, out_features=n_DNN3),nn.BatchNorm1d(num_features=n_DNN3), nn.ReLU())
        self.output = nn.Linear(in_features=n_DNN3, out_features=1)
    ...
    for DNN2 in DNN2s:
        ...
        for DNN3 in DNN3s:
            ...
            for i in range(5):
                ...
                best_test = []
                losses = nn.BCELoss()
                model = My_Net(DNN1, DNN2, DNN3).to(device)
                ...
                for epoch in range(num_epoches):
                    ...
                    if epoch == 299:
                        output_score = save_output(output_score, i, best_auc, best_test)

            dict = get_eval_dict(output_score, DNN1, DNN2, DNN3)
            record = pd.DataFrame.from_dict(dict, orient='index').T
            ...
            all_output_results.to_csv('all_records_DNN1.csv', index=False)
            # all_output_results.to_csv('all_records_DNN2.csv', index=False)
            # all_output_results.to_csv('all_records_DNN3.csv', index=False)
