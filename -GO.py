...
class Other_Embedding(nn.Module):
    ...
    def forward(self, Disease_feature, Pathway_feature):
...
class My_Net(nn.Module):
    ...
    def forward(self, x_dr, x_p, finger_feats, seq_feats, disease_feat, Pathway_feat, my_G, Path2P_G):
        ...
        p_new = torch.cat((h_p_seq, h_p1, h_p2, h_p_pathway), dim=1)
...
for data_type in data_types:
    for type in types:
        for i in range(5):
            ...
            for epoch in range(num_epoches):
                ...
                for step, (batch_x, batch_y) in enumerate(train_loader):
                    ...
                    output = model(b_x_dr, b_x_p, finger_feats, seq_feats, disease_feats, pathway_feat, my_G, Path2P_G)
        ...
        pd_output.to_csv(data_type + type + '_scores_without_GO.csv')
