...
for train_index, dev_index in skf.split(X, Y):
    ...
    with torch.no_grad():
        for step, (batch_x, batch_y) in enumerate(test_loader):
            ...
            b_x_dr = torch.reshape(b_x[:, 0], (len(b_x), 1))
            b_x_p = torch.reshape(b_x[:, 1], (len(b_x), 1))
            output = my_model(b_x_dr, b_x_p, finger_feats, seq_feats, disease_feats, MF_feat, BP_feat, CC_feat,
                              MF_sim_Graph, BP_sim_Graph, CC_sim_Graph, GO2P_G, my_G)
            score = torch.squeeze(output, dim=1)
...
all_output_pandas.to_csv("All_scores_10fold_new.csv", index=False)
