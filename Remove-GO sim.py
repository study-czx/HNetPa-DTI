...
class My_Net(nn.Module):
    ...
    def forward(self, x_dr, x_p, finger_feats, seq_feats,  disease_feat, MF_feat, BP_feat, CC_feat, GO2P_G, my_G):
        ...
        # enhance GO terms
        # pass GO to protein
        h_p_GO = self.GO2P(h_p_seq, h_mf, h_bp, h_cc, GO2P_G)
        # HGNNs in HNet-DrPD
        ...
        for i in range(5):
            ...
            for epoch in range(num_epoches):
                ...
                for step, (batch_x, batch_y) in enumerate(train_loader):
                    ...
                    output = model(b_x_dr, b_x_p, finger_feats, seq_feats, disease_feats, MF_feat, BP_feat, CC_feat, GO2P_G, my_G)
                ...
                with torch.no_grad():
                    ...
                    for step, (batch_x, batch_y) in enumerate(test_loader):
                        ...
                        b_x_dr = torch.reshape(b_x[:, 0], (len(b_x), 1))
                        b_x_p = torch.reshape(b_x[:, 1], (len(b_x), 1))
                        output = model(b_x_dr, b_x_p, finger_feats, seq_feats, disease_feats, MF_feat, BP_feat,CC_feat, GO2P_G,
                                       my_G)
                        score = torch.squeeze(output, dim=1)
                        scores, label = score.cpu().detach().numpy(), b_y.cpu().detach().numpy()
