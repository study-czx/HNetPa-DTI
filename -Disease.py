...
class Other_Embedding(nn.Module):
    ...
    def forward(self, MF_feature, BP_feature, CC_feature, Pathway_feature):
...
class My_Net(nn.Module):
    ...
    def forward(self, x_dr, x_p, finger_feats, seq_feats, MF_feat, BP_feat, CC_feat,
                Pathway_feat, GO2P_G, DDI_G, PPI_G, Path2P_G):
        ...
        h_mf, h_bp, h_cc, h_path = self.other_embedding(MF_feat, BP_feat, CC_feat, Pathway_feat)
...
for data_type in data_types:
    for type in types:
        for i in range(5):
            ...
            for epoch in range(num_epoches):
                ...
                for step, (batch_x, batch_y) in enumerate(train_loader):
                    ...
                    output = model(b_x_dr, b_x_p, finger_feats, seq_feats,
                                   MF_feat, BP_feat, CC_feat, pathway_feat, GO2P_G, DDI_G, PPI_G, Path2P_G)
                ...
                with torch.no_grad():
                    ...
                    for step, (batch_x, batch_y) in enumerate(dev_loader):
                        ...
                        b_x_p = torch.reshape(b_x[:, 1], (len(b_x), 1))
                        output = model(b_x_dr, b_x_p, finger_feats, seq_feats,
                                       MF_feat, BP_feat, CC_feat, pathway_feat, GO2P_G, DDI_G, PPI_G,
                                       Path2P_G)
                        score = torch.squeeze(output, dim=1)
                        scores, label = score.cpu().detach().numpy(), b_y.cpu().detach().numpy()
        ...
        pd_output.to_csv(data_type + type + '_scores_without_disease.csv')
