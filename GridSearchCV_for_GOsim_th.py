...
wd = 1e-6
b_size = 64
ths = [1, 0.5, 0.1, 0.05, 0.01, 0.005, 0.001]
...
def get_eval_dict(this_output_score, epoch_type, th):
    ...
    this_dict = {'lr': lr, 'wd': wd, 'b_size': b_size, 'th': th, 'num_epoches': epoch_type, 'mean_dev_auc': mean_dev_auc,
...
for th in ths:
    print('th: ', th)
    MF_sim_origin = np.loadtxt(r"./GO/GO_sim/MF_sim_1849.csv", dtype=float, delimiter=",", skiprows=1)
    BP_sim_origin = np.loadtxt(r"./GO/GO_sim/BP_sim_5199.csv", dtype=float, delimiter=",", skiprows=1)
    CC_sim_origin = np.loadtxt(r"./GO/GO_sim/CC_sim_744.csv", dtype=float, delimiter=",", skiprows=1)
    # print(MF_sim_origin)
    new_MF_sim = funcs.delete_smalle_sim(MF_sim_origin, th)
    new_BP_sim = funcs.delete_smalle_sim(BP_sim_origin, th)
    new_CC_sim = funcs.delete_smalle_sim(CC_sim_origin, th)
    ...
    dict_50 = get_eval_dict(output_score_epoch_50, epoch_type=50, th=th)
    dict_100 = get_eval_dict(output_score_epoch_100, epoch_type=100, th=th)
    dict_150 = get_eval_dict(output_score_epoch_150, epoch_type=150, th=th)
    dict_200 = get_eval_dict(output_score_epoch_200, epoch_type=200, th=th)
    dict_250 = get_eval_dict(output_score_epoch_250, epoch_type=250, th=th)
    dict_300 = get_eval_dict(output_score_epoch_300, epoch_type=300, th=th)
    ...
    all_output_results.to_csv('all_records_different_GO_th.csv', index=False)
